...
public class SwanLake3197timeLimit {
	static int count = 0;
	...
	static void search(int x, int y, char[][] meltMap) {
		boolean flag = false;
		for (int i = 0, cx = 0, cy = 0; i < dirX.length; i++) {
			cx = x+dirX[i];
			cy = y+dirY[i];
			if (cx < 0 || cy < 0 || cx > arr.length-1 || cy > arr[x].length-1) continue;
	...
	static void melt() {
		char[][] meltMap = new char[arr.length][arr[0].length];
		for (int i = 0; i < arr.length; i++) {
			for (int j = 0; j < arr[i].length; j++) {
				if (arr[i][j] == 'X') {
					search(i, j, meltMap);
				}
				else meltMap[i][j] = arr[i][j];
			}
		}
		for (int i = 0; i < arr.length; i++) {
			arr[i] = meltMap[i].clone();
		}
	}
	static void bfs(int swanX1, int swanY1, int swanX2, int swanY2) {
		...
		Queue<LocLimit> q = new LinkedList<LocLimit>();
		LocLimit l = new LocLimit(swanX1, swanY1);
		q.add(l);
